//  gcc -o ex11-01c ex11-01c.c && gcc -o ex11-01 ex11-01.c && ./ex11-01    
...
int main() {
    ...
    if (pid1 > 0 && pid2 > 0) { /* parent */
        ...
        while(select(p2[0]+1, &newset, NULL, NULL, NULL) > 0) {
            if (FD_ISSET(p1[0], &newset) && read(p1[0], msg, MSGSIZE) > 0)
                printf("[parent] %s\n", msg);
        ...
        dup2(p1[1], 1);
        execl("ex11-01c", "ex11-01xc", (char *)0);
    } else if (pid1 > 0 && pid2 == 0) { /* 2nd child */
        printf("[fork2] hello!\n");
        close(p1[0]);
        close(p1[1]);
        close(p2[0]);
